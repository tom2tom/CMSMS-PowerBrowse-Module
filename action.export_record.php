...
//handles just a single record, so don't bother with export direct to file

if(!($this->CheckAccess('admin') || $this->CheckAccess('view'))) exit;

$funcs = new pwbrExport();
$reportString = $funcs->CSV($this,FALSE,$params['record_id']);
if($reportString)
{
...
$this->Redirect($id,'browse_list',$returnid,array(
